...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* pan-y (not none): a day with many games still has to scroll vertically
     ...
     .details-body (not just .schedule-tab) since that's what the swipe
     listener is actually attached to (see initScheduleTab()) - a short
     day's list leaves empty space below .schedule-tab itself that's still
     part of .details-body, and without this the browser's own native
     touch-action:auto there can grab a horizontal drag before JS gets a
     chance to, making swipe silently not work only in that empty area. */
