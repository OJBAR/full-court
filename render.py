...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* Inline, next to the tricode - not stacked above it - so the logo+code
     form a single-line unit and the score in the middle of the row lines
     up against that unit's own vertical center (.game-row's
     align-items:flex-start then keeps that shared line at the top,
     matching the .team-record line that may follow below it - see that
     rule's own comment). */
  .team-logo {{ display: inline-block; width: 18px; height: 18px; vertical-align: middle; margin-inline-end: 4px; }}
