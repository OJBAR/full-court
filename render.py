...
<body>
  ...
  <script>
    ...
    (function initCupBracketPager() {{
      ...
      // getBoundingClientRect() reports the real on-screen (post-zoom) size,
      // but this track lives inside a .strip-viewport that initFitToWidth()
      // may have scaled down with CSS zoom - and a transform on an element
      // inside a zoomed ancestor is ITSELF rendered at that same scale (a
      // translateX(100px) inside a 0.63 zoom actually moves 63px on screen).
      // Mixing a zoomed width measurement into an unzoomed transform value
      // undershoots every step by the zoom factor - not obvious until you
      // measure it, but very visible on a real device (a sliver of the
      // previous column stays on screen, a gap opens up on the other side).
      // Dividing back out by the current zoom keeps unitShift() in the same
      // (unzoomed) space the translateX below actually operates in.
      function unitShift() {{
        var track = tracks[0];
        var firstCol = track && track.children[0];
        if (!firstCol) return 0;
        ...
        var zoom = (viewport && parseFloat(viewport.style.zoom)) || 1;
        return (firstCol.getBoundingClientRect().width / zoom) + gap;
      }}

      // A CSS transition on transform silently never applies here - verified
      // directly (not guessed): with transition set, the computed transform
      // stays at the identity matrix indefinitely no matter how long you
      // wait, even though the exact same transform value applies instantly
      // and correctly with transition:none. Reproduced with a raw style
      // change outside any of this pager's own logic, so it isn't a bug in
      // goTo()/setTracks() - it's transform-transitions genuinely not
      // composing with an ancestor's CSS zoom (the .strip-viewport
      // initFitToWidth() scales) in this engine. The playoff bracket's
      // pager doesn't use zoom at all, which is exactly why its own
      // animated transition (still present there) has never shown this.
      // Snapping instantly instead of sliding is a real trade-off, but a
      // pager that silently doesn't move at all is worse.
      function setTracks(px) {{
        tracks.forEach(function(track) {{
          track.style.transition = "none";
          track.style.transform = "translateX(-" + px + "px)";
        }});
