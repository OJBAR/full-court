...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  :root.tabs-mode .app-home {{ display: block; }}
  ...
  :root.tabs-mode .header {{ cursor: pointer; }}
...
<body>
  ...
  <script>
    ...
    (function initInstallBanner() {{
      ...
      text.innerHTML = "<strong>הוסיפו את Full Court למסך הבית</strong>תצוגה מותאמת מובייל וגישה מהירה - ישר מהמסך הבית";
    ...
    (function initAppHome() {{
      ...
      // header or the summary itself (see below). Listens on the whole
      // header, not just the logo <img> itself, so it always fires
      // regardless of exactly where within the logo/title/date area the
