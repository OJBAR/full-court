...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* Cup-specific: the Quarterfinal round's own pairs get the same 78px
     internal gap the Semifinal pair already uses, purely for visual
     breathing room (so tapping/swiping the Cup bracket isn't confined to a
     cramped block) - which then means the Semifinal gap has to be
     recalculated the same way .bracket-pair-r2's own comment above derives
     78 in the first place, just with this bracket's real numbers: a
     Quarterfinal pair's own height is now matchHeight(56) + this new
     78px gap = 134px, and the two conferences' Quarterfinal pairs are
     stacked 134 + .bracket-conf-block's own 18px margin + 134 apart -
     so the Semifinal pair's two slots need gap = 56 + 78 + 18 = 152 to
     land on those same two centers. Estimated the same way the original
     78/78 pairing was (not visually re-verified against a live connector
     line render), same caveat as everywhere else this project derives
     bracket geometry by formula. */
  .cup-bracket-pager .bracket-pair:not(.bracket-pair-r2) {{ gap: 78px; }}
  .cup-bracket-pager .bracket-pair-r2 {{ gap: 152px; }}
  ...
  /* The Cup pager's own viewport (see initCupBracketPager()) used to be
     scaled down by initFitToWidth()'s CSS zoom, but that turned out both
     unnecessary (it shares the exact same column width/viewport constraint
     as the playoff bracket's own pager, which was never scaled and fits
     fine) and the source of real bugs - translateX() on a zoomed element
     wasn't reliable on real iOS Safari at all, confirmed directly on a
     real device, not just theorized. Real native scrolling doesn't have
     that problem regardless - overflow-x here overrides the shared
     .strip-viewport rule above (which stays hidden/transform-based for the
     playoff bracket's own pager). scroll-snap-type (with
     scroll-snap-align below, on the first two columns only - snapping to
     the last column would leave dead space past it) handles a free-drag
     swipe settling on a whole step natively, instead of a hand-rolled JS
     "wait for scroll events to stop, then compute the nearest step"
     listener, which turned out to fight the very same scrollTo() calls
     initCupBracketPager() itself makes (a smooth scrollTo()'s real
     duration isn't fixed, so guessing when it's "done" to stop reacting to
     its own scroll events was never reliable - confirmed directly, not
     assumed). Scrollbar hidden since the prev/next buttons plus the
     pager-arrow visual language already the established way to show this
     is scrollable. */
  .cup-bracket-pager .strip-viewport {{
    overflow-x: auto;
    overflow-y: hidden;
    -webkit-overflow-scrolling: touch;
    scrollbar-width: none;
    scroll-snap-type: x mandatory;
    /* The shared .strip-viewport rule above sets touch-action:pan-y, which
       made sense for the playoff bracket's own pager (custom JS captures
       horizontal drags itself, pan-y just lets vertical page-scroll pass
       through) - but here it actively blocks the whole point of switching
       to real native scrolling, since pan-y tells the browser horizontal
       touch gestures aren't this element's to handle. Overridden back to
       the default so an actual swipe here works like any other native
       horizontal scroller. */
    touch-action: auto;
  }}
  .cup-bracket-pager .strip-viewport::-webkit-scrollbar {{ display: none; }}
  .cup-bracket-pager .strip-track {{ transition: none; }}
  .cup-bracket-pager .bracket-column:nth-child(1),
  .cup-bracket-pager .bracket-column:nth-child(2) {{ scroll-snap-align: start; }}
...
<body>
  ...
  <script>
    ...
    (function initCupBracketPager() {{
      // Real native horizontal scrolling instead of a JS-transformed track -
      // rewritten after confirming directly on a real device that
      // translateX() on an element inside a CSS-zoomed viewport simply
      // doesn't work reliably (not just the animated case, the same
      // zoom/transform interaction wasn't trustworthy full stop), back when
      // this pager's own viewport was still scaled down by
      // initFitToWidth(). That scaling turned out to be unnecessary
      // entirely - this pager uses the exact same column width and
      // viewport constraint as the playoff bracket's own pager, which was
      // never scaled and fits fine - and removing it removed the whole
      // zoom-coordinate-space problem category along with it. The column
      // width (146) and gap (24, see .strip-track/.bracket-column above)
      // are still used directly here rather than measured, since they're
      // fixed, authored CSS values, not something that varies per device -
      // simpler and no less reliable than measuring them live.
      var wrap = document.querySelector(".cup-bracket-pager");
      if (!wrap) return;

      var viewports = Array.prototype.slice.call(wrap.querySelectorAll(".strip-viewport"));
      var track = viewports[0] && viewports[0].querySelector(".strip-track");
      if (!viewports.length || !track) return;
      ...
      var syncing = false;
      function mirrorScroll(source) {{
        if (syncing) return;
        syncing = true;
        viewports.forEach(function(vp) {{
          if (vp !== source) vp.scrollLeft = source.scrollLeft;
        }});
        syncing = false;
      }}
      viewports.forEach(function(vp) {{
        vp.addEventListener("scroll", function() {{ mirrorScroll(vp); }}, {{ passive: true }});
      }});
      ...
      // behavior:"smooth" turned out not to be trustworthy together with
      // scroll-snap-type:mandatory here either - confirmed directly, not
      // assumed: a smooth scrollTo() back to an earlier step visibly
      // updated the step/button state but the actual scroll position never
      // moved at all, while the exact same call with behavior:"auto"
      // (instant) worked correctly every time. Same trade-off already made
      // once for this bracket's pan (see the CSS zoom/transition comment
      // above) - snapping instantly instead of sliding, because a pager
      // that updates its own state without visibly moving is worse than
      // one that isn't animated.
      function goTo(newStep) {{
        step = Math.max(0, Math.min(newStep, maxStep));
        var px = step * unitShift();
        viewports.forEach(function(vp) {{
          vp.scrollTo({{ left: px, behavior: "auto" }});
        }});
        updateButtons();
      }}
      ...
      // A free-drag swipe already lands on a whole step by itself - CSS
      // scroll-snap (see .strip-viewport/.bracket-column above) handles
      // that natively, no JS needed. This only keeps `step` (and the
      // prev/next buttons' disabled state) in sync with wherever it
      // settled, once scrolling genuinely stops - it never calls
      // scrollTo() itself, so there's nothing here for it to fight with
      // goTo()'s own scrollTo() calls (an earlier version tried to guard
      // against exactly that fight with a timer-based flag, but a smooth
      // scrollTo()'s real duration isn't fixed, so guessing when it was
      // "done" to stop reacting to its own scroll events was never
      // reliable - confirmed directly, not assumed).
      var settleTimer = null;
      viewports[0].addEventListener("scroll", function() {{
        if (settleTimer) clearTimeout(settleTimer);
        settleTimer = setTimeout(function() {{
          var unit = unitShift();
          step = unit ? Math.round(viewports[0].scrollLeft / unit) : 0;
          updateButtons();
        }}, 150);
      }}, {{ passive: true }});

      window.addEventListener("resize", function() {{ goTo(step); }});
      // scrollTo() on a hidden element (this pager's own <details> starts
      // closed) is a silent no-op - confirmed directly, not assumed: the
      // very first goTo() below runs before the user has ever opened this
      // tab, so it has no visible effect at all, and nothing re-runs it once
      // the tab actually opens. A native "toggle" listener doesn't catch
      // this in tabs-mode - also checked directly: tabs-mode's own
      // <details> stays open="true" from the very first render and is
      // shown/hidden by promoting/repositioning it instead (see
      // initAppHome()), toggling its class list, not its open state, so
      // "toggle" never fires. IntersectionObserver would be the more
      // obvious tool for "did this become visible", but it didn't fire at
      // all when checked directly against this exact element either - a
      // plain MutationObserver on the class attribute did fire correctly,
      // so that's what's used: on any class change, re-run goTo() if the
      // element now actually has real size (still nothing to do while it's
      // hidden).
