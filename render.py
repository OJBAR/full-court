...
<body>
  ...
  <script>
    ...
    (function initPlayoffBracketPager() {{
      ...
      // A round's own .bracket-pair::after connector stub (25px, or 33px
      // for a round-2 pair) reaches PAST the column's own measured width,
      // into the gap - column-width+gap alone lands the pan exactly at the
      // column's edge, leaving a few px of that now-disconnected stub still
      // poking into view past the strip viewport's own clip edge. This is a
      // FLAT one-time extra (added once, not per step - see its one caller
      // below) since it only ever needs to clear the single trailing stub
      // at whatever the current step's own edge is, not accumulate with
      // every step taken; folding it into unitShift() itself (multiplied by
      // step) overshot at step 2, clipping content that should stay
      // visible. 0 at step 0 - there's no previous round's stub to clear
      // yet there.
      var STUB_CLEAR_PX = 45;
      ...
      function totalShift() {{
        // Reverted to plain step * unitShift() (no extra stub-clearing
        // offset) - two attempts at compensating for the connector stub's
        // small cosmetic overhang (per-step-multiplied, then flat) both
        // introduced worse, real content clipping (the NBA Finals box's own
        // right edge cut off at step 2) instead of just hiding the stub.
        // The stub nub is a known, deferred cosmetic issue - see CLAUDE.md
        // backlog - not worth re-risking real clipping to chase further
        // without being able to properly re-derive the Finals track's own
        // width math first.
        return step * unitShift();
      }}
