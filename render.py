...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  :root.tabs-mode .wrapper {{ padding-bottom: 72px; }}
  :root.tabs-mode details.tab-section summary {{ display: none; }}
  .bottom-tabs {{ display: none; }}
  :root.tabs-mode .bottom-tabs {{
    display: flex;
    position: fixed;
    bottom: 0;
    left: 0;
  ...
  .bottom-tab-btn.active {{ color: var(--accent); font-weight: 700; background: var(--bg); }}
  ...
  @media (prefers-reduced-motion: no-preference) {{
    ...
    button:active, .bottom-tab-btn:active, summary:active {{ transform: scale(0.96); }}
    button, .bottom-tab-btn {{ transition: transform 0.1s ease; }}
  }}
</style>
</head>
<body>
  ...
  <script>
    ...
    (function initBottomTabs() {{
      ...
      bar.setAttribute("aria-label", "ניווט בין הסיכומים");

      sections.forEach(function(section, i) {{
        var label = section.querySelector("summary").textContent;
        var btn = document.createElement("button");
        btn.type = "button";
        btn.className = "bottom-tab-btn" + (i === 0 ? " active" : "");
