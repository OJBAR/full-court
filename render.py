...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* pan-y (not none): a day with many games still has to scroll vertically
     ...
     own touch handling (swipe between days), scoped to .schedule-tab itself
     only (see initScheduleTab()'s touchArea comment for why the wider
     scopes tried before this were reverted). */
...
<body>
  ...
  <script>
    ...
    (function initScheduleTab() {{
      ...
      // Deliberately scoped to just .schedule-tab (not .details-body or the
      // whole <details>) - both wider scopes were tried, to reach the empty
      // space below a short day's list, and both caused the sticky tab
      // title bar to visibly jump on iOS (confirmed via real screenshots,
      // not just this session's own limited testing tool). Trading full-
      // screen swipe coverage for a title bar that reliably stays put - the
      // nav buttons still reach every day either way.
