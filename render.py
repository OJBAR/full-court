...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* The Cup pager's own viewport (see initCupBracketPager()) is scaled down
     by initFitToWidth()'s CSS zoom, and translateX() on a zoomed element
     turned out not to be reliable on real iOS Safari at all - confirmed
     directly on a real device, not just theorized: the transform simply
     never took effect, panel stuck in place no matter how long you waited.
     Real native scrolling doesn't have that problem - overflow-x here
     overrides the shared .strip-viewport rule above (which stays
     hidden/transform-based for the playoff bracket's own pager, unaffected
     since it never used zoom to begin with). scroll-snap-type (with
...
<body>
  ...
  <script>
    ...
    (function initCupBracketPager() {{
      ...
      // translateX() on an element inside this pager's CSS-zoomed viewport
      // (initFitToWidth()) simply doesn't work reliably: not just the
      // animated case (a transition on transform silently never applied),
      // the same zoom/transform interaction turned out not to be trustworthy
      // full stop. scrollLeft doesn't have that problem - but it does have a
      // different one, also confirmed directly rather than assumed:
      // scrollLeft/clientWidth/scrollWidth are measured in the zoomed
      // element's own PRE-zoom ("local") pixels, while getBoundingClientRect()
      // on the exact same element's children reports real POST-zoom screen
      // pixels - two different coordinate spaces on the same zoomed
      // subtree. Measuring a column's rect and feeding that straight into
      // scrollTo() mixes the two and undershoots, the same class of bug as
      // the old translateX one just mentioned. Since the column width (146)
      // and gap (24, see .strip-track/.bracket-column above) are fixed,
      // authored CSS values, not something that varies per device, using
      // them directly here sidesteps the whole zoom-coordinate-space
      // question rather than trying to measure through it.
      ...
      if (detailsEl && window.MutationObserver) {{
        var classObserver = new MutationObserver(function() {{
          if (wrap.getBoundingClientRect().width > 0) goTo(step);
        }});
