...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* pan-y (not none): a day with many games still has to scroll vertically
     ...
     own touch handling (swipe between days). Applied to the whole
     details.tab-section (not just .schedule-tab or .details-body) - that's
     what the swipe listener is actually attached to (see initScheduleTab()),
     since .details-body does NOT reliably stretch to fill the screen here
     (confirmed directly - same <details>-as-flex-child quirk as the Cup
     bracket), while the <details> element itself does. touch-action set on
     an ancestor bounds every descendant's gesture handling too (including
     the sticky summary bar), so this also stops the browser from grabbing
     even the first bit of a horizontal drag natively before our own
     touchmove/preventDefault gets a chance to - a native-scroll-then-
     preventDefault race that showed up as the summary bar visibly jumping
     at the start of a swipe. */
  .schedule-tab {{ touch-action: pan-y; }}
  details.tab-section:has(.schedule-tab) {{ touch-action: pan-y; }}
...
<body>
  ...
  <script>
    ...
    (function initScheduleTab() {{
      ...
      // Listens on the whole open tab <details> (not .details-body, and not
      // just .schedule-tab) so a swipe works even over the empty space below
      // a short day's list. Confirmed directly (via the real tabs-mode
      // summary click, not just .open=true) that .details-body does NOT
      // actually stretch to fill the screen here, despite its own CSS flex:1
      // - same underlying <details>-as-flex-child quirk already hit and
      // worked around for the Cup bracket (see initCupBracketPager's
      // centerVertically()). The <details> element itself DOES reliably
      // fill the screen (its own flex:1 is relative to <main>, a plain
      // element, not nested inside another flex-child-of-<details>
      // relationship), so that's the one this binds to instead. Only the
      // games list itself visually slides, the nav bar stays put.
      ...
      var touchArea = wrap.closest("details.tab-section") || wrap;
      var startX = null;
      var startY = null;
      var dragging = false;
